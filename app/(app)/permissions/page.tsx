...
export default function PermissionsPage() {
  ...
  const handleCreateRequest = async () => {
    ...
    try {
      ...
      if (result.success) {
        showToast('تم إرسال طلب الصلاحية بنجاح', 'ok');
        setShowForm(false);
        setFormContractId('');
        setFormUserId('');
        setFormNotes('');
  ...
  const handleApprove = async (requestId: string) => {
    ...
    if (result.success) {
      showToast('تم اعتماد الطلب وتفعيل الصلاحية', 'ok');
      loadData();
    } else {
  ...
  const handleRevokeApprover = async (approverId: string) => {
    ...
    if (result.success) {
      showToast('تم إلغاء صلاحية المعتمد', 'ok');
      loadData();
    } else {
  ...
  const handleDirectAdd = async () => {
    ...
    try {
      ...
      if (result.success) {
        showToast('تم إضافة المعتمد بنجاح', 'ok');
        setShowForm(false);
        loadData();
      } else {
  ...
  const statusBadge = (status: string) => {
    const map: Record<string, { label: string; variant: 'orange' | 'green' | 'red' | 'gray' }> = {
      pending: { label: 'قيد الانتظار', variant: 'orange' },
      approved: { label: 'معتمد', variant: 'green' },
      rejected: { label: 'مرفوض', variant: 'red' },
    };
    const s = map[status] || { label: status, variant: 'gray' };
    return <CustomBadge label={s.label} variant={s.variant} />;
  };
        ...
        <Card>
          <CardBody>
            ...
            <div className="flex gap-2 mt-3">
              <Button variant="red" onClick={handleReject} disabled={!rejectReason.trim()}>
                تأكيد الرفض
              </Button>
                  ...
                  <tbody>
                    {approvers.map(a => (
                      <tr key={a.id} className="border-b border-gray-100 hover:bg-gray-50">
                        ...
                        <td className="px-3 py-2 text-center">
                          <CustomBadge label={scopeLabel(a.approval_scope)} variant="blue" />
                        </td>
                        <td className="px-3 py-2 text-center text-xs">
