...
export async function sendWorkflowNotification(
  ...
  try {
    ...
    const typeMap: Record<ClaimStatus, NotificationType> = {
      ...
      under_technical_review: 'claim_submitted',
      returned_by_technical: 'claim_returned',
      under_quality_review: 'claim_submitted',
      returned_by_quality: 'claim_returned',
      under_project_manager_review: 'claim_submitted',
      returned_by_project_manager: 'claim_returned',
      returned_by_final_approver: 'claim_returned',
    ...
    const getStageAr = (status: ClaimStatus): string => {
      const stageMap: Record<ClaimStatus, string> = {
        ...
        // Phase 2.6 — quality / technical / PM / final-approver-return stages.
        under_technical_review: 'قيد مراجعة الوحدة الفنية',
        returned_by_technical: 'مرجعة من الوحدة الفنية',
        under_quality_review: 'قيد مراجعة وحدة الجودة',
        returned_by_quality: 'مرجعة من وحدة الجودة',
        under_project_manager_review: 'قيد مراجعة مدير المشروع',
        returned_by_project_manager: 'مرجعة من مدير المشروع',
        returned_by_final_approver: 'مرجعة من الاعتماد النهائي',
